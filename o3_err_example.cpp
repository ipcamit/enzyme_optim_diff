...
inline real cut_cos(real const r, real const rcut) {
    return (r < rcut) ? 0.5 * (cos(MY_PI * r / rcut) + 1.0) : 0.0;
}
void sym_g2(real const eta, real const Rs, real const r, real const rcut, real &phi) {
    phi = exp(-eta * (r - Rs) * (r - Rs)) * cut_cos(r, rcut);
}

VectorXreal symmetry_function_atomic(int const i,
                              VectorXreal coords,
                              int const *particleSpeciesCodes,
                              int const *neighlist,
                              int const numnei,
    ...
    // prepare data
    VectorXreal desc(51);
    for (int jj = 0; jj < 51; jj++) {desc[jj] = 0.;};
    ...
    for (int jj = 0; jj < numnei; ++jj) {
        ...
        // cutoff between ij
        real rcutij = SymParam->rcut_2D_(iSpecies, jSpecies);

        // Compute rij
        real rij[DIM];
        for (int dim = 0; dim < DIM; ++dim) {
            rij[dim] = coords[j*3 +  dim] - coords[i*3 +  dim];
        }

        real const rijsq = rij[0] * rij[0] + rij[1] * rij[1] + rij[2] * rij[2];
        real const rijmag = sqrt(rijsq);
        ...
        for (std::size_t p = 0; p < SymParam->name_.size(); ++p) {
            ...
            for (int q = 0; q < SymParam->num_param_sets_[p]; ++q) {
                real gc = 0.0;
                    real eta = SymParam->params_[p](q, 0);
...
int main() {
    ...
    Eigen::Map<VectorXreal> coords_vec(coords, 259 * 3);
    Eigen::Map<Eigen::VectorXd> forces_vec(forces, 259 * 3);
    // Eigen::Map<VectorXreal> desc_vec  (desc, 102);
    Eigen::Map<Eigen::VectorXd> d_desc_vec  (d_desc, 102);
    VectorXreal desc_vec;
    ...
    for(int i = 0; i < 2; i++) {
        ...
        neigh_from += n_neigh_;
        Eigen::MatrixXd J = jacobian(symmetry_function_atomic, wrt(coords_vec), at(i, coords_vec, species, n_list.data(), n_neigh_, &sf), desc_vec);
        // auto desc_vec = symmetry_function_atomic(i, coords_vec, species, n_list.data(), n_neigh_, &sf);
        forces_vec += J.transpose() * d_desc_vec(Eigen::seqN(i * 51,51));
    }
